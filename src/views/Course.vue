...
<template>
  <div class="container">
    ...
    <el-main class="main">
      <!-- Banner区域 -->
      <div class="banner">
        <el-image src="https://cdn8.axureshop.com/wp-content/uploads/2025/06/110646-111057790415-722x361.png"
          alt="Banner" fit="cover" class="banner-content" />
      </div>
      <!-- 专业方向和学时筛选区域 -->
      <div class="filter">
        <div>
          <div class="filter-item">
            ...
            <el-checkbox-group type="text" v-model="searchCondition.list">
              <el-checkbox-button v-for="(direction, index) in subjects" :key="index" :value="direction"
                @change="handleData">{{ direction }}</el-checkbox-button>
            </el-checkbox-group>
          </div>
          <div class="filter-item">
            <div class="title">学时</div>
            <el-checkbox-group type="text" v-model="searchCondition.range">
              <el-checkbox-button v-for="(hours, index) in classHour" :key="index" :value="hours"
                @change="handleData">{{ hours }}学时</el-checkbox-button>
            </el-checkbox-group>
          </div>
        </div>
        <div>
          <el-button type="text" size="small" class="sort-button" @click="handleSort">
            发布时间<el-icon>
              <ArrowUp v-if="searchCondition.sort === 'asc'" />
              <ArrowDown v-else-if="searchCondition.sort === 'desc'" />
            </el-icon>
          </el-button>
        </div>
      </div>
      ...
      <div class="course-list" v-loading="courses.length === 0">
        <CourseCard v-for="(item, index) in courses" :key="index" :course="item" @click="handleCourse(item)"
          class="course-item" />
      </div>
